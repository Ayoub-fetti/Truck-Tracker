...
exports.getTrailers = async (req, res) => {
  ...
  const trailers = await Trailer.find(filter).populate('truckAttache', 'immatriculation');
  res.json(trailers);
};

exports.getTrailer = async (req, res) => {
  const trailer = await Trailer.findById(req.params.id).populate('truckAttache', 'immatriculation');
  if (!trailer) return res.status(404).json({ message: 'Trailer not found' });
  res.json(trailer);
};

exports.updateTrailer = async (req, res) => {
  const trailer = await Trailer.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
  if (!trailer) return res.status(404).json({ message: 'Trailer not found' });
  res.json(trailer);
};
