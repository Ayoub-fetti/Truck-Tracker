...
exports.getTrucks = async (req, res) => {
  ...
  const trucks = await Truck.find(filter).populate('chauffeurAssigne', 'nom email');
  res.json(trucks);
};

exports.getTruck = async (req, res) => {
  const truck = await Truck.findById(req.params.id).populate('chauffeurAssigne', 'nom email');
