...
exports.generateTripPDF = async (req, res) => {
  ...
  if (trip.dateArrivee) doc.text(`Date arrivée: ${trip.dateArrivee.toLocaleDateString()}`);
  doc.text(`Kilométrage départ: ${trip.kilometrageDepart} km`);
  if (trip.kilometrageArrivee) doc.text(`Kilométrage arrivée: ${trip.kilometrageArrivee} km`);
  if (trip.distanceParcourue) doc.text(`Distance parcourue: ${trip.distanceParcourue} km`);
