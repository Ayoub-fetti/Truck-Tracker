...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    try {
      const [trucks, trailers, trips, maintenance, tires, fuel] = await Promise.all([
      ...
      setStats({
        ...
        activeTrips: trips.data.filter(trip => trip.status === 'active').length,
        pendingMaintenance: maintenance.data.filter(m => m.status === 'pending').length,
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold text-green-600">{stats.totalTrailers}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold">Active Trips</h3>
          <p className="text-3xl font-bold text-purple-600">{stats.activeTrips}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold">Pending Maintenance</h3>
          <p className="text-3xl font-bold text-red-600">{stats.pendingMaintenance}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold">Total Tires</h3>
          <p className="text-3xl font-bold text-yellow-600">{stats.totalTires}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold">Fuel Logs</h3>
          <p className="text-3xl font-bold text-indigo-600">{stats.totalFuelLogs}</p>
        </div>
      </div>
    </div>
  );
